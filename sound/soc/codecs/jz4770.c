...
static int jz_icdc_pcm_trigger(struct snd_pcm_substream *substream,
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream != SNDRV_PCM_STREAM_PLAYBACK) {
			snd_soc_codec_force_bias_level(codec, SND_SOC_BIAS_ON);
			msleep(2);
		}
...
{
	return snd_soc_codec_force_bias_level(codec, SND_SOC_BIAS_OFF);
}

static int jz_icdc_resume(struct snd_soc_codec *codec)
{
	return snd_soc_codec_force_bias_level(codec, SND_SOC_BIAS_STANDBY);
}
...
static int jz_icdc_codec_probe(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_dapm_new_controls(dapm, widgets, ARRAY_SIZE(widgets));
	if (ret)
		return ret;
...
static int jz_icdc_codec_remove(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_codec_force_bias_level(codec, SND_SOC_BIAS_OFF);
...
static int jz_icdc_probe(struct platform_device *pdev)
{
	...
	jz_icdc->base = devm_ioremap_resource(&pdev->dev, mem);
	if (IS_ERR(jz_icdc->base)) {
		ret = PTR_ERR(jz_icdc->base);
		dev_err(&pdev->dev, "Failed to ioremap mmio memory: %d\n", ret);
		return ret;
	}
