...
static int __devinit jz4770_pcm_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &jz4770_soc_platform);
}

static int __devexit jz4770_pcm_remove(struct platform_device *pdev)
{
...
static struct platform_driver jz4770_pcm_driver = {
	...
	.remove = __devexit_p(jz4770_pcm_remove),
	.driver = {
