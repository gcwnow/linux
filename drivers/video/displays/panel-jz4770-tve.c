...
static int jz4770_tve_panel_init(void **out_panel, struct device *dev,
				 void *panel_pdata)
{
	struct jz4770_tve *panel = &jz4770_tve_panel;
	...
	panel->base = devm_ioremap_resource(dev, mem);
	if (IS_ERR(panel->base)) {
		dev_err(&pdev->dev, "Failed to ioremap TVE regs: %ld\n",
			PTR_ERR(panel->base));
