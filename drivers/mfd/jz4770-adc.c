...
static inline void jz4770_adc_set_enabled(struct jz4770_adc *adc, int engine,
	bool enabled)
{
	unsigned long flags;
	uint8_t val;
...
static int jz4770_adc_cell_enable(struct platform_device *pdev)
{
	...
	jz4770_adc_set_enabled(adc, pdev->id, true);
...
static int jz4770_adc_cell_disable(struct platform_device *pdev)
{
	...
	jz4770_adc_set_enabled(adc, pdev->id, false);
...
static int __devinit jz4770_adc_probe(struct platform_device *pdev)
{
	struct irq_chip_generic *gc;
...
static int __devexit jz4770_adc_remove(struct platform_device *pdev)
{
...
static struct platform_driver jz4770_adc_driver = {
	...
	.remove = __devexit_p(jz4770_adc_remove),
	.driver = {
		.name = "jz4770-adc",
		.owner = THIS_MODULE,
	},
