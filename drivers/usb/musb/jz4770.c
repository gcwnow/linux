...
static void do_otg_id_pin_state(struct musb *musb)
{
	...
	unsigned int default_a = !gpiod_get_value(glue->gpio);
...
static void otg_id_pin_stable_func(struct timer_list *t)
{
...
static int otg_id_pin_setup(struct musb *musb)
{
	...
	glue->gpio = devm_gpiod_get(dev->parent, NULL, GPIOD_IN);
	if (IS_ERR(glue->gpio)) {
		ret = PTR_ERR(glue->gpio);
	...
	do_otg_id_pin_state(musb);
	timer_setup(&musb->dev_timer, otg_id_pin_stable_func, 0);

	ret = devm_request_irq(dev, gpiod_to_irq(glue->gpio), jz_musb_otg_id_irq,
...
static void otg_id_pin_cleanup(struct musb *musb)
{
	...
	devm_free_irq(dev, gpiod_to_irq(glue->gpio), musb);
